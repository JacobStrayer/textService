...
function sendText(phone, message, carrier, region, cb) {
  ...
  // for each possible cell phone carrier, send them an email to see if it'll send a message to the number
  const p = Promise.all(providersList.map((provider) => {
    const to = provider.replace('%s', phone);
    // let temp = message + provider;
    const mailOptions = {
      to,
      subject: null,
      text: message,
    ...
    return new Promise((resolve, reject) => transporter.sendMail(mailOptions, (err, info) => {
      if (err) {
        console.log('error in the promise', err.message);
        return reject(err)
      };
      return resolve(info);
    }));
